...
class AI_News_Assistant_Backend_Provider extends AI_News_Assistant_Provider_Base {
	...
	public function analyze_source( $url ) { return $this->request( 'analyze_source', array( 'url' => $url ) ); }
	public function generate( array $input ) {
		$data = $this->request( 'generate_news', $input );
		if ( is_wp_error( $data ) ) return $data;
		return $this->map_article( $data, $input );
	}
	public function generate_article( array $input ) { $data = $this->request( 'generate_article', array( 'title' => $input['title'] ?? '', 'payload' => $input ) ); return is_wp_error( $data ) ? $data : $this->map_article( $data, $input ); }
	public function generate_image( array $input ) { return $this->request( 'generate_image', $input ); }
	private function map_article( array $data, array $input ) { $mapped = array( 'main_title' => $data['title'] ?? '', 'alternative_titles' => $data['alternative_titles'] ?? array(), 'lead' => $data['lead'] ?? '', 'content' => $data['content_html'] ?? '', 'summary_points' => $data['summary_points'] ?? array(), 'verification_notes' => $data['verification_notes'] ?? array(), 'fact_checklist' => $data['fact_checklist'] ?? array(), 'seo' => array( 'seo_title' => $data['seo_title'] ?? '', 'meta_description' => $data['meta_description'] ?? '', 'slug' => $data['slug'] ?? '', 'focus_keyword' => $data['focus_keyword'] ?? '', 'tags' => $data['tags'] ?? array(), 'category_suggestion' => $data['category_suggestion'] ?? '' ), 'seo_audit' => $data['seo_audit'] ?? array(), 'source_attribution' => $data['source_attribution'] ?? ( $input['source_analysis'] ?? array() ), 'similarity' => $data['similarity'] ?? array(), 'social_captions' => $data['social_captions'] ?? array(), 'review_status' => $data['review_status'] ?? 'Needs Verification', 'usage' => $data['usage'] ?? array() ); return $this->normalize( $mapped, $input ); }
	private function request( $type, array $payload ) {
		...
		if ( $site_token ) { $paths = array( 'detect_news_type' => 'detect-news-type', 'analyze_source' => 'analyze-source', 'generate_news' => 'generate-news', 'generate_article' => 'generate-article', 'generate_image' => 'generate-image' ); if ( ! isset( $paths[ $type ] ) ) return new WP_Error( 'aina_backend_request', __( 'Jenis request AI tidak didukung.', 'ai-news-assistant' ) ); $url = $base . '/api/ai/' . $paths[ $type ]; $body = $payload; $headers = array( 'Authorization' => 'Bearer ' . $site_token, 'X-Site-URL' => home_url(), 'Content-Type' => 'application/json', 'Accept' => 'application/json' ); }
