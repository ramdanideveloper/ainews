...
class AI_News_Assistant_Backend_Provider extends AI_News_Assistant_Provider_Base {
	...
	private function request( $type, array $payload ) {
		...
		if ( $site_token ) { $paths = array( 'detect_news_type' => 'detect-news-type', 'generate_news' => 'generate-news', 'generate_article' => 'generate-article', 'generate_image' => 'generate-image' ); if ( ! isset( $paths[ $type ] ) ) return new WP_Error( 'aina_backend_request', __( 'Jenis request AI tidak didukung.', 'ai-news-assistant' ) ); $url = $base . '/api/ai/' . $paths[ $type ]; $body = $payload; $headers = array( 'Authorization' => 'Bearer ' . $site_token, 'X-Site-URL' => home_url(), 'Content-Type' => 'application/json', 'Accept' => 'application/json' ); }
