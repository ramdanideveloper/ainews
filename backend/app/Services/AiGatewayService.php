...
class AiGatewayService
{
    ...
    public function image(array $payload, ConnectedSite $site): array
    {
        ...
        $prompt = $payload['custom_prompt'] ?? trim(($payload['title'] ?? '').' '.($payload['content_summary'] ?? '').' Keyword: '.($payload['keyword'] ?? '').' Style: '.$style.'. No text, watermark, logo, gore, or identifiable minor.');
        try {
            $result = $this->router->image('image_generate', ['prompt' => $prompt, 'size' => $this->size($payload['aspect_ratio'] ?? '1:1')]);
