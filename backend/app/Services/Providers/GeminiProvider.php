...
class GeminiProvider implements AiProviderInterface
{
    public function text(AiProvider $p, array $messages, array $options = []): array
    {
        ...
        $r = Http::connectTimeout(15)->timeout(120)->withToken($p->api_key)->post($url, ['model' => $model, 'messages' => $messages, 'temperature' => $options['temperature'] ?? 0.3, 'reasoning_effort' => $options['reasoning_effort'] ?? 'low', 'response_format' => ['type' => 'json_object']]);
        if (! $r->successful()) {
