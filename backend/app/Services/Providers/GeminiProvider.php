...
class GeminiProvider implements AiProviderInterface
{
    ...
    {
        $base = rtrim($p->base_url ?: 'https://generativelanguage.googleapis.com/v1beta/openai', '/');
        $r = Http::timeout(60)->withToken($p->api_key)->post($base.'/chat/completions', ['model' => $p->model_id, 'messages' => $messages, 'temperature' => $options['temperature'] ?? 0.3, 'response_format' => ['type' => 'json_object']]);
        if (! $r->successful()) {
            throw new RuntimeException((string) ($r->json('error.message') ?: 'Gemini request failed with HTTP '.$r->status()));
        }$j = $r->json();
